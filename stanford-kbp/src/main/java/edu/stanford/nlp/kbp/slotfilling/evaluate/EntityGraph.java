...
public class EntityGraph extends DirectedMultiGraph<KBPEntity, KBPSlotFill> {
  ...
  public void add(KBPEntity source, KBPEntity destination, KBPSlotFill edge) {
    ...
    if (!edge.key.slotValue.equals(destination.name)) {
      throw new IllegalArgumentException("Destination does not match edge's destination. destination=" + source + " edge=" + edge);
    }
    ...
    List<KBPSlotFill> mergableEdges = new ArrayList<>();
...
//              warn(String.format("Types do not match!: %s vs %s and %s vs %s", head, head_, tail, tail_ ) );
  ...
  public Set<KBPEntity> getConnectedComponent(KBPEntity root) {
    Set<KBPEntity> component = new IdentityHashSet<>();
    component.add(root);
    dfs(component, root);
  ...
  public void restrictGraph(Set<KBPEntity> mainComponent) {
    List<KBPEntity> complement = new ArrayList<>();
  ...
  public double getScore(KBTriple triple) {
    for (KBPSlotFill edge : this.getEdges(triple.getEntity(), triple.getSlotEntity().orNull())) {
      if (edge.key.equals(triple)) { return edge.score.getOrElse(1.0); }
    }
    return 0.0;
  }
