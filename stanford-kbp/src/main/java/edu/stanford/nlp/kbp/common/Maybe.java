...
public abstract class Maybe<T> implements Iterable<T>, Serializable {
  ...
  private Maybe(){ } // Hide constructor
  ...
  public static <T> Maybe<T> Nothing() { return uncheckedCast(nothingSingleton); }

  /** Create a new Maybe with its value filled */
  public static <T> Maybe<T> Just(T x) { return new Just<T>(x); }

  public static <E> Maybe<E> fromString(String serialized) {
  ...
  public static <E> Maybe<E> fromNull(E elemOrNull) {
