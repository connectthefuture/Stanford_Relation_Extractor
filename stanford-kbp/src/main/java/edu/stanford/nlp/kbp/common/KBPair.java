...
public class KBPair implements Serializable, Comparable<KBPair> {
  ...
  public KBPProtos.KBTuple toProto() {
    KBPProtos.KBPEntity.Builder slotBuilder = KBPProtos.KBPEntity.newBuilder().setName(this.slotValue);
    for (NERTag type : this.slotType) { slotBuilder.setType(type.name); }
    KBPProtos.KBTuple.Builder builder = KBPProtos.KBTuple.newBuilder();
    return builder
        .setEntity(this.getEntity().toProto())
        .setSlotValue(slotBuilder.build())
        .build();
  }
  ...
  public boolean equals(KBPair other) {
    return this.getEntity().equals(other.getEntity())
        ...
        && (
            (this.slotType.isDefined() && other.slotType.equalsOrElse(this.slotType.get(), false)) ||
            (other.slotType.isDefined() && this.slotType.equalsOrElse(other.slotType.get(), false)) ||
            (!this.slotType.isDefined() && !other.slotType.isDefined())
           );
  }
  ...
  /**
   * @see edu.stanford.nlp.kbp.common.KBPair#getEntity()
   */
  private volatile KBPEntity cachedEntityView = null;
  /**
   * @see edu.stanford.nlp.kbp.common.KBPair#getSlotEntity()
   */
  ...
  /**
   * Similar to {@link edu.stanford.nlp.kbp.common.KBPair#getSlotEntity()}, but forces the NER type of the
   * slot fill to be MISC if no NER type is known.
   * @return The slot fill, with a potentially forced NER type.
   */
