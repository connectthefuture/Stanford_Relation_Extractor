...
public abstract class OfficialOutputWriter {
  ...
  public void outputRelations(PrintStream os, String runId, Map<KBPOfficialEntity, Collection<KBPSlotFill>> rawRelations, Map<RelationType, Double> threshold) {
    ...
    // query id
    List<KBPOfficialEntity> entities = new ArrayList<>();
    ...
    for (KBPOfficialEntity entity : entities) {
      ...
      Map<String, List<KBPSlotFill>> mentionMap = new HashMap<>();
      for (KBPSlotFill mention : mentions) {
        String relationName = officialRelationName(mention.key.kbpRelation());
        if (!mentionMap.containsKey(relationName)) mentionMap.put(relationName, new ArrayList<>());
        mentionMap.get(relationName).add(mention);
      }

      Set<String> relationsOutput = new HashSet<>();
   ...
   * @return A valid String to be printed in the
   */
  protected String spansToOffsetString(String docid, Span... spans) {
    if (spans.length > 2) {
      throw new IllegalArgumentException("Too many spans for offset");
    }
    StringBuilder b = new StringBuilder();
    for (int i = 0; i < spans.length; ++i) {
      b.append(docid).append(":").append(spans[i].start()).append("-").append(spans[i].end());
      if (i < spans.length - 1) { b.append(","); }
    }
    return b.toString();
  }
...
//        return " ";
  ...
  public static class OfficialOutputWriter2013 extends OfficialOutputWriter {
    ...
    protected Span characterOffset(List<CoreLabel> tokens, Span tokenOffset) {
    ...
    /** Offsets for a particular well-defined span */
    protected Span characterOffset(CoreMap sentence, Span tokenOffset) {
    ...
    protected Span characterOffset(CoreMap sentence, KBPRelationProvenance provenance,
     ...
     */
    protected Maybe<Span> resolveLiteralCoref(Annotation doc, KBTriple key, boolean entity) {
      if (key.getSlotEntity().isDefined()) {
        new PostIRAnnotator(KBPNew.from(key.getEntity()).KBPOfficialEntity(), key.getSlotEntity().get(), true).forceLink().annotate(doc);
      } else {
        new PostIRAnnotator(KBPNew.from(key.getEntity()).KBPOfficialEntity(), true).forceLink().annotate(doc);
      }
      if (entity) {
    ...
    protected Span resolveDocDate(Annotation doc, Span defaultReturnValue) {
    ...
    public void yieldLine(StringBuilder builder, KBPSlotFill fill) throws IOException {
      ...
      // Get Sentences
      Annotation doc = ir.officialIndex.fetchDocument(primaryProvenance.docId).orCrash("Could not find document in official index: " + primaryProvenance.docId);
      ...
      if (fill.key.kbpRelation() == RelationType.ORG_ALTERNATE_NAMES || fill.key.kbpRelation() == RelationType.PER_ALTERNATE_NAMES) {
        ...
        builder.append(spansToOffsetString(
            characterOffset(primarySentence, primaryProvenance,
                primaryEntityOffset, primaryValueOffset)
        )).append(tab());
  ...
  /**
   * A writer to output the official response files for 2014
   */
  public static class OfficialOutputWriter2014 extends OfficialOutputWriter2013 {
    public OfficialOutputWriter2014(StandardIR ir) {
      super(ir);
    }
    ...
    public void yieldLine(StringBuilder builder, KBPSlotFill fill) throws IOException {
      ...
      // Column 4: Provenance
      // (relation provenance)
      Span relationProvenance = characterOffset(primarySentence, primaryProvenance,
          primaryEntityOffset, primaryValueOffset);
      builder.append(spansToOffsetString(primaryProvenance.docId, relationProvenance));
      // (entity antecedent)
      if (!relationProvenance.contains(entityAntecedent)) {
        builder.append(",").append(spansToOffsetString(primaryProvenance.docId, entityAntecedent));
      }
      // (slot value antecedent)
      if (!relationProvenance.contains(valueAntecedent)) {
        builder.append(",").append(spansToOffsetString(primaryProvenance.docId, valueAntecedent));
      }
      // (final tab)
      builder.append(tab());
      ...
      if (!primaryValueOffset.equals(valueAntecedent)) {
        // case: coreferent
        builder.append(spansToOffsetString(primaryProvenance.docId, primaryValueOffset, valueAntecedent));
      } else if (!primaryValueOffset.equals(docDate)) {
        builder.append(spansToOffsetString(primaryProvenance.docId, primaryValueOffset, docDate));
      } else if (secondaryProvenance.isDefined() && secondarySentence.isDefined()) {
        // case: multiple provenances
        builder.append(spansToOffsetString(primaryProvenance.docId,
            primaryValueOffset,
            characterOffset(secondarySentence.get(), secondaryProvenance.get().slotValueMentionInSentence.orCrash())
        ));
      } else {
        // case: single provenance
        builder.append(spansToOffsetString(primaryProvenance.docId, primaryValueOffset));
      }
      builder.append(tab());
