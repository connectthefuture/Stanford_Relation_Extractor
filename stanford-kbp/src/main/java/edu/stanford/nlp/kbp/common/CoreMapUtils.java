...
public class CoreMapUtils {
  ...
  public static Annotation copyDocument( Annotation document ) {
    ...
    List<CoreMap> newSentences = new ArrayList<>( sentences.size() );
  ...
  public static CoreMap copySentence( CoreMap sentence ) {
    ...
    List<CoreLabel> newTokens = new ArrayList<>( tokens.size() );
  ...
  private static class SentenceTextOffsets {
    ...
    public String toString(){
      return "[sentenceBegin = " + sentenceBegin+", sentenceEnd = " + sentenceEnd + ", entityBegin = " +
        entityBegin + ", entityEnd = " + entityEnd + ", slotFillBegin = " + slotFillBegin + ", slotFillEnd = " + slotFillEnd;
    }
  }
  ...
  public static String documentToMinimalString(Annotation annotation) {
    return StringUtils.join(
        CollectionUtils.map(annotation.get(CoreAnnotations.SentencesAnnotation.class),
            CoreMapUtils::sentenceToMinimalString)
    );
  }
  ...
  public static List<CoreLabel> tokenize(String text) {
    ...
    tokenizer = new PTBTokenizer<>(sr, tokenFactory, options);
  ...
  /* Beware of changing me! You may invalidate the sentencegloss cache */
  public static String getSentenceGlossKey(String[] sentencegloss, Span entitySpan, Span valueSpan) {
    return getHexKeyString(StringUtils.join(sentencegloss, "~#~")) + ":" +
        entitySpan.start() + "-" + entitySpan.end() + ":" +
        valueSpan.start() + "-" + valueSpan.end();
  }

  /* Beware of changing me! You may invalidate the sentencegloss cache */
  public static String getSentenceGlossKey(List<CoreLabel> sentencegloss, Span entitySpan, Span valueSpan) {
    String[] tokens = new String[sentencegloss.size()];
    for (int i = 0; i < sentencegloss.size(); ++i) {
      CoreLabel token = sentencegloss.get(i);
      if (token.containsKey(CoreAnnotations.OriginalTextAnnotation.class)) {
        tokens[i] = token.originalText();
      } else {
        tokens[i] = token.word();
      }
    }
    return getSentenceGlossKey(tokens, entitySpan, valueSpan);
  }
  ...
  public static String[] phraseToOriginalTokens(List<CoreLabel> coreLabels) {
  ...
  /**
   * Start walking up the tree until you hit the given relation.
   * If you hit that relation, return true. If you never hit the relation, return false.
   * However, if you ever reach the blockOn relation, do not continue up the tree.
   *
   * @param graph The dependency graph to walk up.
   * @param node The node to start walking up from.
   * @param relation The relation to search for. This is really a case-insensitive substring of the relation.
   * @param blockOn The relation to block on. The search will not continue up from this relation.
   * @return True if the relation was hit going up the tree, and false otherwise.
   */
  private static boolean passesRelationToRoot(SemanticGraph graph, IndexedWord node, String relation, String blockOn) {
    // Pass up towards the root
    for (SemanticGraphEdge edge : graph.getIncomingEdgesSorted(node)) {
      if (edge.getRelation().getShortName().toLowerCase().contains(relation.toLowerCase())) {
        return true;
      } else if (!edge.getRelation().getShortName().toLowerCase().contains(blockOn.toLowerCase()) &&
          passesRelationToRoot(graph, edge.getSource(), relation, blockOn)) {
        return true;
      }
    }
    // Check the relation right below the root, in case of a flipped edge
    if (graph.getIncomingEdgesSorted(node).size() == 0) {
      for (SemanticGraphEdge out : graph.getOutEdgesSorted(node)) {
        if (!out.getTarget().tag().startsWith("V") && out.getRelation().getShortName().toLowerCase().contains(relation.toLowerCase())) {
          return true;
        }
      }
    }
    // The relation wasn't found
    return false;
  }

  /**
   * Returns true if this node is the subject of the sentence expressed by the graph.
   */
  public static boolean isSubject(SemanticGraph graph, IndexedWord node) {
    return passesRelationToRoot(graph, node, "subj", "obj");
  }

  /**
   * Returns true if this node is the object of the sentence expressed by the graph.
   */
  public static boolean isObject(SemanticGraph graph, IndexedWord node) {
    return passesRelationToRoot(graph, node, "obj", "subj");
  }
}
