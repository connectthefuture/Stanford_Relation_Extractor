...
public class EntityMentionAnnotator implements Annotator {
  ...
  public static List<EntityMention> extractEntityMentions(KBPEntity entity, CoreMap sentence) {
    ...
    Set<Span> literalMatches = new HashSet<>();
    int entityIndex = 0;
    // Loop over tokens
    for (int i = 0; i < tokens.size(); ++i) {
      ...
      if (entityIndex >= entityTokens.length) {
        ...
        if (Props.KBP_VERBOSE) { logger.debug("found entity mention [direct]: " + em); }
        entities.add(em);
        // Mark span as filled
    ...
    Set<Integer> candidateCorefStarts = new HashSet<>();
    ...
    Set<Span> corefSpans = new HashSet<>();
    ...
    for (Span corefSpan : corefSpans) {
      ...
      assert Utils.getNERTag(em).isDefined();
      em.setNormalizedName(entity.name);
      if (Props.KBP_VERBOSE) { logger.debug("found entity mention [coref]: " + em); }
      entities.add(em);
    }
