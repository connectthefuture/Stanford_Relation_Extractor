...
public class KBPSlotValidator extends KBPEvaluator {
   ...
   *
   * @see KBPEvaluator#KBPEvaluator(java.util.Properties, edu.stanford.nlp.kbp.common.Lazy, edu.stanford.nlp.kbp.common.Lazy, edu.stanford.nlp.kbp.common.Lazy)
   */
  public KBPSlotValidator(Properties props, KBPIR ir, KBPProcess process, RelationClassifier classify) {
    super(props, Lazy.from(ir), Lazy.from(process), Lazy.from(classify));
  ...
  private static boolean approximateEntityMatchScore( String higherGloss, String lowerGloss) {
    return Props.ENTITYLINKING_LINKER.get().sameEntity(
  ...
  private KBPSlotFillQuery constructQuery(String validationQueryID, KBPOfficialEntity entity, RelationType relation, String docid,
    ...
    Annotation doc = null;
    try {
      doc = irComponent.get().fetchDocument(docid.trim());
      new PostIRAnnotator(entity, true)
          .annotate(doc);
      ...
      for (Span fillerSpan : fillerSpans) {
        for (Span entitySpan : entitySpans) {
          ...
          if (sentenceSpan.contains(fillerSpan) && sentenceSpan.contains(entitySpan)) {
            ...
            List<CoreMap> validSentences = processComponent.get().annotateSentenceFeatures(entity, containingSentences);
            if (validSentences.size() > 0) {
              for (SentenceGroup datum : processComponent.get().featurizeSentence(validSentences.get(0), Maybe.<RelationFilter>Nothing())) {
                ...
                if (datum.key.entityName.equals(entity.name) &&  // entities match
                    (approximateEntityMatchScore(datum.key.slotValue, queryString) ||  // strings are close enough
                        ...
                        AcronymMatcher.isAcronym(entity.name, tokens.subList(slotValueTokenSpan.start(), slotValueTokenSpan.end())) ||
                        AcronymMatcher.isAcronym(queryString, entity.name.split("\\s+"))
                    )) {
                  // Classify datum
                  classifierScore = Maybe.Just(classifier.get().classifyRelation(datum, relation, Maybe.<CoreMap[]>Nothing()).first);
                  Counter<Pair<String, Maybe<KBPRelationProvenance>>> predictions = classifier.get().classifyRelations(datum, Maybe.<CoreMap[]>Nothing());
