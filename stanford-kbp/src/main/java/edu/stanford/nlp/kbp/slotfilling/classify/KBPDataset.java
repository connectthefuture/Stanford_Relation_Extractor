...
public class KBPDataset<L, F> implements Serializable {
  ...
  */
   ...
   */
  protected final Map<Pair<Integer,Integer>, L>  annotatedLabels = new HashMap<>();
  ...
  public Set<L> getPositiveLabels(int i) {
    ...
    return getLabels(positiveIndices);
  }
  ...
  public Set<L> getNegativeLabels(int i) {
    ...
    return getLabels(negativeIndices);
  }
  ...
  private Set<L> getLabels(Set<Integer> indices) {
  ...
  public List<Datum<L, F>> getDatumGroup(int i) {
    List<Datum<L, F>> result = new ArrayList<>();
...
//
  public void addDatum(Set<L> yPos, Set<L> yNeg, Set<L> yUnk, List<Datum<L, F>> group, Maybe<? extends List<String>> sentenceGlossKeys,
                       Maybe<L>[] annotatedLabels) {
    List<Collection<F>> features = new ArrayList<>();
    for(Datum<L, F> datum: group){
      features.add(datum.asFeatures());
    }
    add(yPos, yNeg, yUnk, features, sentenceGlossKeys, annotatedLabels);
  }

  private void add(Set<L> yPos, Set<L> yNeg, Set<L> yUnk, List<Collection<F>> group,
                   Maybe<? extends List<String>> sentenceGlossKeys, Maybe<L>[] annotatedLabels) {
    ...
    // Copy annotated labels
    for (int sentenceI = 0; sentenceI < annotatedLabels.length; ++sentenceI) {
      if (annotatedLabels[sentenceI].isDefined()) {
        this.annotatedLabels.put(Pair.makePair(size, sentenceI), annotatedLabels[sentenceI].get());
      }
    }
   ...
   * @param annotatedLabels An array containing known annotated labels, which should be used to augment the {@link KBPDataset#annotatedLabels}
   *                        of this dataset with the newly added data.
   */
  public void addDatum(Set<Integer> yPos, Set<Integer> yNeg, Set<Integer> yUnk, int[][] group, String[] keys,
                       Maybe<L>[] annotatedLabels) {
    ensureSize();
    
    addPosLabelIndices(new HashSet<>(yPos));
    addNegLabelIndices(new HashSet<>(yNeg));
    addUnkLabelIndices(new HashSet<>(yUnk));
