...
public class KBPNew {
  ...
  public static class KBPEntityBuilder {
    ...
    public KBPOfficialEntityBuilder representativeDocument(final EntityContext doc) { return new KBPOfficialEntityBuilder(this) {{ this.representativeDocument = doc; }}; }
    public KBPOfficialEntityBuilder representativeDocument(final Maybe<EntityContext> doc) { return new KBPOfficialEntityBuilder(this) {{ this.representativeDocument = doc.orNull(); }}; }
  ...
  public static class KBPOfficialEntityBuilder extends KBPEntityBuilder {
    ...
    protected EntityContext representativeDocument = null;
    ...
    protected KBPOfficialEntityBuilder(KBPOfficialEntity ent) {
      ...
      this.representativeDocument = ent.representativeContext.orNull();
    }
    ...
    @Override
    public KBPOfficialEntityBuilder representativeDocument(EntityContext doc) { this.representativeDocument = doc; return this; }
    @Override
    public KBPOfficialEntityBuilder representativeDocument(Maybe<EntityContext> doc) { this.representativeDocument = doc.orNull(); return this; }
    // Discharges
    @Override
  ...
  public static class KBPairBuilder extends KBPOfficialEntityBuilder {
    ...
    public KBPairBuilder ent(final KBPEntity ent) { this.name = ent.name; this.type = ent.type; this.id = ent instanceof KBPOfficialEntity ? ((KBPOfficialEntity) ent).id.orNull() : null; return this; }
    @Override
    ...
    @Override
    public KBPairBuilder representativeDocument(EntityContext doc) { this.representativeDocument = doc; return this; }
    @Override
    public KBPairBuilder representativeDocument(Maybe<EntityContext> doc) { this.representativeDocument = doc.orNull(); return this; }
    @Override
  ...
  public static class KBTripleBuilder extends KBPairBuilder {
    ...
    @Override
    public KBTripleBuilder representativeDocument(EntityContext doc) { this.representativeDocument = doc; return this; }
    @Override
    public KBTripleBuilder representativeDocument(Maybe<EntityContext> doc) { this.representativeDocument = doc.orNull(); return this; }
    @Override
    public KBTripleBuilder ent(final KBPEntity ent) { this.name = ent.name; this.type = ent.type; this.id = ent instanceof KBPOfficialEntity ? ((KBPOfficialEntity) ent).id.orNull() : null; return this; }
    @Override
    ...
    public KBTriple KBTriple() {
      ...
      return new KBTriple(Maybe.fromNull(id), name, type, relation, slotValue, Maybe.fromNull(slotType));
    }
  ...
  public static class KBPSlotFillBuilder extends KBTripleBuilder {
    ...
    protected KBPSlotFillBuilder(KBTripleBuilder tripleBuilder) {
      super(tripleBuilder);
    }
    ...
    public KBPSlotFillBuilder entity(KBPEntity entity) {
      ...
      if (entity instanceof KBPOfficialEntity) {
        ...
        this.representativeDocument = p.representativeContext.orNull();
      }
      return this;
    }
    @Override
    ...
    @Override
    public KBPSlotFillBuilder representativeDocument(EntityContext doc) { this.representativeDocument = doc; return this; }
    @Override
    public KBPSlotFillBuilder representativeDocument(Maybe<EntityContext> doc) { this.representativeDocument = doc.orNull(); return this; }
    @Override
  ...
  /**
   * Create either a {@link edu.stanford.nlp.kbp.common.KBPEntity} or {@link edu.stanford.nlp.kbp.common.KBPOfficialEntity}
   * from the given prtocol buffer.
   *
   * @param entity The protocol buffer to parse.
   * @return A {@link edu.stanford.nlp.kbp.common.KBPEntity}, either an official entity or a simple entity depending on the
   * fields in the proto.
   */
  public static KBPOfficialEntityBuilder from(KBPProtos.KBPEntity entity) {
    KBPOfficialEntityBuilder builder = new KBPOfficialEntityBuilder(KBPNew.entName(entity.getName()).entType(entity.getType()));
    if (entity.hasId()) { builder = builder.entId(entity.getId()); }
    if (entity.hasQueryId()) { builder = builder.queryId(entity.getQueryId()); }
    if (entity.hasRepresentativeDocument()) { builder = builder.representativeDocument(EntityContext.fromProto(entity.getRepresentativeDocument())); }
    List<String> ignoredSlots = entity.getIgnoredSlotsList();
    if (ignoredSlots.size() > 0) {
      Set<RelationType> relations = new HashSet<>();
      for (String rel : ignoredSlots) { relations.add(RelationType.fromString(rel).orCrash()); }
      builder = builder.ignoredSlots(relations);
    }
    return builder;
  }

  /**
   * Create either a {@link edu.stanford.nlp.kbp.common.KBPair} or {@link edu.stanford.nlp.kbp.common.KBTriple}
   * from the passed protocol buffer.
   *
   * @param tuple The protocol buffer to parse, representing a knowledge base entry.
   * @return A KBPair if no relation is provided in the protocol buffer, else a KBTriple.
   */
  public static KBTripleBuilder from(KBPProtos.KBTuple tuple) {
    KBPEntity entity = from(tuple.getEntity()).KBPEntity();
    KBPairBuilder builder = KBPNew.from(entity).slotValue(tuple.getSlotValue().getName());
    if (tuple.getSlotValue().hasType()) { builder = builder.slotType(tuple.getSlotValue().getType()); }
    if (tuple.hasRelation()) {
      return builder.rel(tuple.getRelation());
    } else {
      return builder.rel((String) null);
    }
  }

  /**
   * Create a {@link edu.stanford.nlp.kbp.slotfilling.ir.KBPRelationProvenance} from the passed protcol
   * buffer.
   *
   * @param fill The protocol buffer to parse.
   * @return A slot fill object representing the given protocol buffer
   */
  public static KBPSlotFillBuilder from(KBPProtos.KBPSlotFill fill) {
    KBTripleBuilder builder = KBPNew.from(from(fill.getKbEntry()).KBTriple());
    if (fill.hasProvenance()) {
      KBPSlotFillBuilder fillBuilder = builder.provenance(KBPRelationProvenance.fromProto(fill.getProvenance()));
      if (fill.hasScore()) { fillBuilder.score(fill.getScore()); }
      return fillBuilder;
    }
    return new KBPSlotFillBuilder(builder);
  }
