...
public class HeuristicSlotfillPostProcessors {
  ...
  public static class FilterIgnoredSlots extends HeuristicSlotfillPostProcessor.Default {
    ...
    public Maybe<KBPSlotFill> isValidSlotAndRewrite(KBPEntity pivot, KBPSlotFill candidate) {
      if (pivot instanceof KBPOfficialEntity && candidate.key.hasKBPRelation()) {
        return ((KBPOfficialEntity) pivot).ignoredSlots.getOrElse(new HashSet<>()).contains(candidate.key.kbpRelation()) ? singletonFailure(candidate, this.getClass()): Maybe.Just(candidate);
      } else {
        return Maybe.Just(candidate);
      }
    }
  ...
  public static class SanityCheckFilter extends HeuristicSlotfillPostProcessor.Default {
    public Maybe<KBPSlotFill> isValidSlotAndRewrite(KBPEntity pivot, KBPSlotFill candidate) {
      ...
      try {
        if (candidate.key.kbpRelation() == RelationType.PER_AGE && candidate.key.slotValue.contains(".")) { return singletonFailure(candidate, this.getClass()); }
        if (candidate.key.kbpRelation() == RelationType.PER_AGE && (NumberNormalizer.wordToNumber(candidate.key.slotValue).intValue() > 125 || NumberNormalizer.wordToNumber(candidate.key.slotValue).intValue() <= 0)) { return singletonFailure(candidate, this.getClass()); }
      } catch (NumberFormatException | NullPointerException ignored) { }
      // Filter unreasonable employee counts
      // (assumes we're never talking about companies with < 25 employees)
      try {
        if (candidate.key.kbpRelation() == RelationType.ORG_NUMBER_OF_EMPLOYEES_SLASH_MEMBERS && candidate.key.slotValue.contains(".")) { return singletonFailure(candidate, this.getClass()); }
        if (candidate.key.kbpRelation() == RelationType.ORG_NUMBER_OF_EMPLOYEES_SLASH_MEMBERS && (NumberNormalizer.wordToNumber(candidate.key.slotValue).intValue() > 500000 || NumberNormalizer.wordToNumber(candidate.key.slotValue).intValue() < 25)) { return singletonFailure(candidate, this.getClass()); }
      } catch (NumberFormatException | NullPointerException ignored) { }
  ...
  public static class ConformToGuidelinesFilter extends HeuristicSlotfillPostProcessor.Default {
    ...
    public static final Set<String> invalidTitles = new HashSet<String>() {{
      add("senior leader"); add("leader"); add("member"); add("hero"); add("socialite"); add("rep.");
    }};

    // A set of terms denoting "divisions" or sub-departments that aren't ok for top employees.
    // These are case sensitive
    public static final Set<String> divisionKeywords = new HashSet<String>() {{
      add("division"); add("Division");
      add("department"); add("Department");
      add("branch"); add("Branch");
      // These are common terms, so we're case sensitive
      add("Unit");
      add("Office");
    }};
    ...
    public Maybe<KBPSlotFill> isValidSlotAndRewrite(KBPEntity pivot, KBPSlotFill candidate) {
      ...
      Maybe<String> gloss = Maybe.Nothing();
      if (candidate.provenance.isDefined() && candidate.provenance.get().containingSentenceLossy.isDefined() &&
        ...
        spanBetweenArgs = Maybe.Just(CoreMapUtils.sentenceSpanString(candidate.provenance.get().containingSentenceLossy.get(),
            new Span(Math.min(entitySpan.end(), slotSpan.end()), Math.max(entitySpan.start(), slotSpan.start()))));
        gloss = Maybe.Just(CoreMapUtils.sentenceToMinimalString(candidate.provenance.get().containingSentenceLossy.get()));
      }
        ...
        for (RelationType rel : candidate.key.tryKbpRelation()) {
          switch (rel) {
            case ORG_TOP_MEMBERS_SLASH_EMPLOYEES:
              if (invalidTopEmployeeJustification.contains(spanToLower)) {
                return singletonFailure(candidate, this.getClass());
              }
              for (String sentenceGloss : gloss) {
                for (String keyword : divisionKeywords) {
                  if (sentenceGloss.contains(keyword)) {
                    return singletonFailure(candidate, this.getClass());
                  }
                }
              }
            ...
            case PER_EMPLOYEE_OF:
              if (candidate.key.slotValue.equalsIgnoreCase("House")) {  // "House" is not a fully-specified organization
                return singletonFailure(candidate, this.getClass());
              }
            default:
              break;
      ...
      for (RelationType rel : candidate.key.tryKbpRelation()) {
        if (rel.isDateRelation()) {
          ...
          // Fail year-less times in general
          if (!YEAR.matcher(candidate.key.slotValue).find()) { return singletonFailure(candidate, this.getClass()); }
...
//              !token.containsKey(CoreAnnotations.AntecedentAnnotation.class)) { // and has no antecedent
        ...
        if (newStart != slotSpan.start()) {
          ...
          for (int i = newStart; i < slotSpan.start(); ++i) {
  ...
  /** Rewrite top employees to "founded by." Also, some other sanity checks... */
  public static class TopEmployeeToFounderRewrite extends HeuristicSlotfillPostProcessor.Default {
    @Override
    public Maybe<KBPSlotFill> isValidSlotAndRewrite(KBPEntity pivot, KBPSlotFill candidate) {
      if (candidate.key.hasKBPRelation() &&
          candidate.key.kbpRelation() != RelationType.ORG_TOP_MEMBERS_SLASH_EMPLOYEES) {
        return Maybe.Just(candidate);
      }
  ...
  /** Rewrite age if it's not a year value */
  public static class AgeRewrite extends HeuristicSlotfillPostProcessor.Default {
    @Override
    public Maybe<KBPSlotFill> isValidSlotAndRewrite(KBPEntity pivot, KBPSlotFill candidate) {
      if (!candidate.key.hasKBPRelation() || candidate.key.kbpRelation() != RelationType.PER_AGE) {
        return Maybe.Just(candidate);
      }
      ...
      if (candidate.provenance.isDefined() && candidate.provenance.get().containingSentenceLossy.isDefined() &&
          ...
          candidate.provenance.get().slotValueMentionInSentence.isDefined()) {
        KBPRelationProvenance provenance = candidate.provenance.get();
        List<CoreLabel> lossySentence = provenance.containingSentenceLossy.get().get(CoreAnnotations.TokensAnnotation.class);
        Span slotSpan = provenance.slotValueMentionInSentence.get();
        for (int i = slotSpan.end(); i < Math.min(lossySentence.size(), slotSpan.end() + 5); ++i) {
          try {
            switch (lossySentence.get(i).word().toLowerCase().replaceAll("s$","")) {
              case "decade":
                return Maybe.Just(KBPNew.from(candidate).slotValue("" + NumberNormalizer.wordToNumber(candidate.key.slotValue).intValue() * 10).KBPSlotFill());
              case "month":
                return Maybe.Just(KBPNew.from(candidate).slotValue("" + NumberNormalizer.wordToNumber(candidate.key.slotValue).intValue() / 12).KBPSlotFill());
              case "week":
                return Maybe.Just(KBPNew.from(candidate).slotValue("" + NumberNormalizer.wordToNumber(candidate.key.slotValue).intValue() * 7 / 365).KBPSlotFill());
              case "day":
                return Maybe.Just(KBPNew.from(candidate).slotValue("" + NumberNormalizer.wordToNumber(candidate.key.slotValue).intValue() / 365).KBPSlotFill());
            }
      ...
      if (candidate.provenance.isDefined() && candidate.provenance.get().containingSentenceLossy.isDefined() &&
        ...
        for (int i : middleWords) {
          ...
          if (word.startsWith("reside") || word.toLowerCase().startsWith("live") || word.toLowerCase().contains("home") ||
              ...
              word.toLowerCase().startsWith("left")) {
  ...
  /** Rewrite a bunch of things to "died in" */
  public static class DiedInRewrite extends HeuristicSlotfillPostProcessor.Default {
    public Maybe<KBPSlotFill> isValidSlotAndRewrite(KBPEntity pivot, KBPSlotFill candidate) {
      ...
      RelationType relation = candidate.key.kbpRelation();
      if (!relation.isBirthRelation() && !relation.isResidenceRelation() && relation != RelationType.PER_EMPLOYEE_OF) {
        return Maybe.Just(candidate);
      }
      // Maybe rewrite to to XOfDeath
      // If we already have provenance...
      if (candidate.provenance.isDefined() && candidate.provenance.get().containingSentenceLossy.isDefined() &&
          candidate.provenance.get().entityMentionInSentence.isDefined() &&
          candidate.provenance.get().slotValueMentionInSentence.isDefined()) {
        ...
        if (middleWords.size() < 10) {
          for (int i : middleWords) {
            String word = lossySentence.get(CoreAnnotations.TokensAnnotation.class).get(i).word();
            if ("died".equals(word) || "death".equals(word)) {
              final KBPSlotFill rewritten;
              if (relation.isCityRelation()) {
                rewritten = KBPNew.from(candidate).rel(RelationType.PER_CITY_OF_DEATH).KBPSlotFill();
              } else if (relation.isRegionRelation()) {
                rewritten = KBPNew.from(candidate).rel(RelationType.PER_STATE_OR_PROVINCES_OF_DEATH).KBPSlotFill();
              } else if (relation.isCountryRelation()) {
                rewritten = KBPNew.from(candidate).rel(RelationType.PER_COUNTRY_OF_DEATH).KBPSlotFill();
              } else if (Utils.geography().isValidCountry(candidate.key.slotValue)) {
                rewritten = KBPNew.from(candidate).rel(RelationType.PER_COUNTRY_OF_DEATH).KBPSlotFill();
              } else if (Utils.geography().isValidRegion(candidate.key.slotValue)) {
                rewritten = KBPNew.from(candidate).rel(RelationType.PER_STATE_OR_PROVINCES_OF_DEATH).KBPSlotFill();
              } else if (Utils.geography().isValidCity(candidate.key.slotValue)) {
                rewritten = KBPNew.from(candidate).rel(RelationType.PER_CITY_OF_DEATH).KBPSlotFill();
              } else {
                return Maybe.Just(candidate);
              }
  ...
  /** Filter likely wrong employee justifications */
  public static class EmployeeFilter extends HeuristicSlotfillPostProcessor.Default {
    @Override
    public Maybe<KBPSlotFill> isValidSlotAndRewrite(KBPEntity pivot, KBPSlotFill candidate) {
      if (candidate.key.hasKBPRelation() &&
          candidate.key.kbpRelation() != RelationType.ORG_TOP_MEMBERS_SLASH_EMPLOYEES &&
          candidate.key.kbpRelation() != RelationType.PER_EMPLOYEE_OF) {
        return Maybe.Just(candidate);
      }
      // Rewrite to FoundedBy
      // If we already have provenance...
      if (candidate.provenance.isDefined() && candidate.provenance.get().containingSentenceLossy.isDefined() &&
        ...
        for (int i = Math.min(entitySpan.end(), slotSpan.end()); i < Math.max(entitySpan.start(), slotSpan.start()); ++i) {
          String word = lossySentence.get(CoreAnnotations.TokensAnnotation.class).get(i).word();
          // "said" is hardly ever an indicator of an employee
          if (word.equalsIgnoreCase("said") || word.equalsIgnoreCase("told") || word.equalsIgnoreCase("called") || word.equalsIgnoreCase("quoted")) {
            // ... except in cases like "said CEO John Smith"
            if (i >= lossySentence.get(CoreAnnotations.TokensAnnotation.class).size() - 1 ||
                !lossySentence.get(CoreAnnotations.TokensAnnotation.class).get(i + 1).ner().equals(NERTag.TITLE.name)) {
              return singletonFailure(candidate, this.getClass());
            }
          }
        }
  ...
  public static class NoDuplicates extends HeuristicSlotfillPostProcessor.Default {
    ...
    public boolean pairwiseKeepLowerScoringFill(KBPEntity pivot, KBPSlotFill higherScoring, KBPSlotFill lowerScoring) {
      ...
      if (lowerScoring.key.hasKBPRelation() && higherScoring.key.hasKBPRelation() &&
          (lowerScoring.key.kbpRelation() == RelationType.PER_EMPLOYEE_OF || lowerScoring.key.kbpRelation() == RelationType.PER_MEMBER_OF) &&
          (higherScoring.key.kbpRelation() == RelationType.PER_EMPLOYEE_OF || higherScoring.key.kbpRelation() == RelationType.PER_MEMBER_OF) &&
          KBPNew.from(lowerScoring).rel(RelationType.PER_EMPLOYEE_OF).KBPSlotFill()
              .equals(KBPNew.from(higherScoring).rel(RelationType.PER_EMPLOYEE_OF).KBPSlotFill())) {
  ...
  public static class NoDuplicatesApproximate extends HeuristicSlotfillPostProcessor.Default {
    ...
    // TODO(gabor) there should really only be one linker. But, here we want "John" and "John Doe" to be the same, in other cases we don't.
    public final Lazy<EntityLinker> duplicateDetector = new Lazy<EntityLinker>() {
      @Override
      protected EntityLinker compute() {
        if (Props.HACKS_FORCEBASELINELINKERINCONSISTENCY) {
          return new EntityLinker.GaborsHackyDuplicateDetector();
        } else {
          return Props.ENTITYLINKING_LINKER.get();
        }
      }
    };
      ...
      if (higherScoring.key.kbpRelation() == lowerScoring.key.kbpRelation()) {
        ...
        for (KBPEntity higherEntity : higherScoring.key.getSlotEntity()) {
          for (KBPEntity lowerEntity : lowerScoring.key.getSlotEntity()) {
              ...
              for (EntityContext higherContext : higherProvenance.toSlotContext(higherEntity, irComponent)) {
                for (KBPRelationProvenance lowerProvenance : lowerScoring.provenance) {
                  for (EntityContext lowerContext : lowerProvenance.toSlotContext(lowerEntity, irComponent)) {
                    if (duplicateDetector.get().sameEntity(higherContext, lowerContext)) {
            ...
            if (duplicateDetector.get().sameEntity(new EntityContext(higherEntity), new EntityContext(lowerEntity))) {
  ...
  public static class DuplicateRelationOnlyInListRelations extends HeuristicSlotfillPostProcessor.Default {
    ...
    public boolean pairwiseKeepLowerScoringFill(KBPEntity pivot, KBPSlotFill higherScoring, KBPSlotFill lowerScoring) {
      ...
      if (higherScoring.key.relationName.equals(lowerScoring.key.relationName) &&
          higherScoring.key.kbpRelation().cardinality == RelationType.Cardinality.SINGLE) {
        // Two values for a single-valued relation
        return nonlocalFailure(lowerScoring, this.getClass());
      }
      return true;
    }
