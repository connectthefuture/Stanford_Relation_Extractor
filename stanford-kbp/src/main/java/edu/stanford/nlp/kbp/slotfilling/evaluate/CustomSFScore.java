...
/**
 * This scorer is in theory identical to SFScore2010, but with minor tweaks to integrate in our system.
 */
public class CustomSFScore {
  ...
  public static Pair<Double, Double> scoreByRelationName(PrintStream os, 
      String responseFile, 
      String keyFile, 
      boolean anydoc, 
      Maybe<Set<String>> scoreOnlyTheseValues,
      Set<String> allQueryIds) throws IOException {
    return scoreByRelationName(os, responseFile, keyFile, false, anydoc, false, null, scoreOnlyTheseValues, allQueryIds);
  }
  ...
  public static Pair<Double, Double> scoreByRelationName(PrintStream os, 
      ...
      Maybe<Set<String>> scoreOnlyTheseValues,
      Set<String> allQueryIds) throws IOException {
    return scoreByRelationName(os, responseFile, keyFile, false, anydoc, false, slots, scoreOnlyTheseValues, allQueryIds);
  }
  ...
  public static Pair<Double, Double> score(
      ...
      Maybe<Set<String>> scoreOnlyTheseValuesMaybe,
      Set<String> allQueryIds) throws IOException {
    // reset
    ...
    if(scoreOnlyTheseValuesMaybe.isDefined())
      os.println("Using " + scoreOnlyTheseValuesMaybe.getOrElse(new HashSet<String>()).size() + " candidates.");
    else 
    ...
    while ((line = keyReader.readLine()) != null) {
      ...
      // mihai
      for(Set<String> scoreOnlyTheseValues : scoreOnlyTheseValuesMaybe){
      ...
      if (anydoc && judgement.containsKey(key) && judgement.get(key) == CORRECT) {  // Change by Gabor
        // don't overwrite correct key
      } else {
        judgement.put(key, jment);
        equivalenceClass.put(key, eclass);
      }
      assert judgement.containsKey(key);
      if (jment == CORRECT) {
...
//    }
    ...
    if(allQueryIds != null) {
      ...
      for(String qid: allQueryIds) {
        // single-valued slots
        for(RelationType relationName: RelationType.singleValuedRelations) {
          String key = qid + ":" + OfficialOutputWriter.officialRelationName(relationName);
          if(query_has_answer.get(key) != null) {
            num_answers ++;
          }
        }
        // list-valued slots
        for(RelationType relationName: RelationType.listValuedRelations) {
          String key = qid + ":" + OfficialOutputWriter.officialRelationName(relationName);
      ...
      for (String responseString : responseList) {
        ...
        if (doc_id.equals("NIL")) {
          ...
          case WRONG:
            num_wrong++;
            symbol = "W";
            os.println("Wrong judgement: " + query + ":" +
                doc_id + " " + answer_string);
            break;
          case REDUNDANT:
            num_redundant++;
          ...
          case CORRECT:
            ...
            if (distincts.contains(E)) {
              ...
              os.println("Redundant judgement: " + query + ":" +
                  doc_id + " " + answer_string);
            } else {
              num_correct++;
              symbol = "C";
  ...
  // Same functionality as score(), but prints out per slot-name results
  public static Pair<Double, Double> scoreByRelationName(
      PrintStream os,
      String responseFile,
      String keyFile,
      boolean trace,
...
//    }
    ...
    //   (correct single-value answers + list-value equivalence classes)
    // Key is slot name
    Counter<RelationType> num_answers = new ClassicCounter<RelationType>();
    ...
    if(allQueryIds != null) {
      ...
      for(String qid: allQueryIds) {
        ...
        for(RelationType relationName: RelationType.singleValuedRelations) {
          String key = qid + ":" + OfficialOutputWriter.officialRelationName(relationName);
          if(query_has_answer.get(key) != null) {
            num_answers.incrementCount(relationName);
          }
        }
        // list-valued slots
        for(RelationType relationName: RelationType.listValuedRelations) {
          ...
          if (query_eclasses.get(key) != null)
            num_answers_to_query = query_eclasses.get(key).size();
          num_answers.incrementCount(relationName, num_answers_to_query);
        }
      }
    }
    ...
    // number of non-NIL responses
    // key is slot name
    Counter<RelationType> num_responses = new ClassicCounter<RelationType>();
    // number of correct non-NIL responses
    // key is slot name
    Counter<RelationType> num_correct = new ClassicCounter<RelationType>();
    // counts for different error types
    // keys are slot names
    Counter<RelationType> num_wrong = new ClassicCounter<RelationType>();  // includes spurious and incorrect
    Counter<RelationType> num_inexact = new ClassicCounter<RelationType>();
    Counter<RelationType> num_redundant = new ClassicCounter<RelationType>();
    String symbol = "?";

    for (String query : svSlots) {
      RelationType relationName = RelationType.fromString(query.split(":",2)[1]).orCrash();
      // mihai: this computes a fake recall, only over slots actually answered
      if (allQueryIds == null && query_has_answer.get(query) != null)
        num_answers.incrementCount(relationName);
      List<String> responseList = response.get(query);
      ...
      if (doc_id.equals("NIL")) {
        ...
        num_responses.incrementCount(relationName);
        Integer J = judgement.get(query + ":" + doc_id + ":" + answer_string);
        if (J == null) {
          os.println ("No judgement for " +
              query + ":" + doc_id + " " + answer_string);
        ...
        switch (j) {
        case WRONG:
          os.println("Single-valued slot tagged 'wrong' in key:" +
              "\t" + query + ":" + doc_id + " " + answer_string);
          num_wrong.incrementCount(relationName);
          symbol = "W";
          break;
        case REDUNDANT:
          os.println
          ("Single-valued slot tagged 'redundant' in key:" +
              "\t" + query + ":" + doc_id + " " + answer_string);
          num_redundant.incrementCount(relationName);
          symbol = "R";
          break;
        case CORRECT:
          num_correct.incrementCount(relationName);
          symbol = "C";
          os.println("Correct judgement: " + query + ":" + 
              doc_id + " " + answer_string);
          break;
        case INEXACT:
          num_inexact.incrementCount(relationName);
          symbol = "X";
          break;
        default:
          os.println ("Invalid judgement " + j);
    ...
    for (String query : lSlots) {
      ...
      for (String responseString : responseList) {
        ...
        if (doc_id.equals("NIL")) {
          ...
          switch (j) {
          case WRONG:
            os.println("List-valued slot tagged 'wrong' in key:" +
                "\t" + query + ":" + doc_id + " " + answer_string);
            num_wrong.incrementCount(relationName);
            symbol = "W";
            break;
          case REDUNDANT:
            os.println("List-valued slot tagged 'redundant' in key:" +
                "\t" + query + ":" + doc_id + " " + answer_string);
            num_redundant.incrementCount(relationName);
            symbol = "R";      // redundant with reference KB
            break;
          case CORRECT:
            Integer E = equivalenceClass.get(key);
            if (distincts.contains(E)) {
              os.println("List-valued slot in existing equivalence class: " +
                  "\t" + query + ":" + doc_id + " " + answer_string);
              num_redundant.incrementCount(relationName);
              symbol = "r";   // redundant with other returned response
            } else {
          ...
          case INEXACT:
            num_inexact.incrementCount(relationName);
            os.println("List-valued slot tagged 'inexact' in key:" +
                "\t" + query + ":" + doc_id + " " + answer_string);
            symbol = "X";
            break;
          default:
    ...
    List<String> sortedRelationNames = new ArrayList<String>(relationNames);
    Collections.sort(sortedRelationNames);
    for(String relationString : sortedRelationNames) {
      RelationType relationName = RelationType.fromString(relationString).orCrash(relationString);
      double _num_answers = num_answers.getCount(relationName);
      double _num_responses = num_responses.getCount(relationName);
      double _num_correct = num_correct.getCount(relationName);
      double _num_redundant = num_redundant.getCount(relationName);
      double _num_wrong = num_wrong.getCount(relationName);
      double _num_inexact = num_inexact.getCount(relationName);
      
      os.println ("\nStats for slot name: "+relationName);
      os.println ("\tNumber of filled slots in key: " + _num_answers);
      os.println ("\tNumber of filled slots in response: " + _num_responses);
      os.println ("\tNumber correct non-NIL: " + _num_correct);
      os.println ("\tNumber redundant: " + _num_redundant);
      os.println ("\tNumber incorrect / spurious: " + _num_wrong);
      os.println ("\tNumber inexact: " + _num_inexact);

      
      double recall = _num_correct / _num_answers;
      double precision = _num_correct / _num_responses;
      double F = (2 * recall * precision) / (recall + precision);
      os.println ("\n\t2010 scores:");
      os.println ("\tRecall: " + _num_correct + " / " + _num_answers + " = " + recall);
      os.println ("\tPrecision: " + _num_correct + " / " + _num_responses + " = " + precision);
      os.println ("\tF1: " + F);
    }
    
    //get total results
    double _num_answers = num_answers.totalCount();
    double _num_responses = num_responses.totalCount();
    double _num_correct = num_correct.totalCount();
    double _num_redundant = num_redundant.totalCount();
    double _num_wrong = num_wrong.totalCount();
    double _num_inexact = num_inexact.totalCount();
  ...
  public static String slotType (String slot) {
    ...
    if (RelationType.singleValuedRelation(slotFields[1]))
      return "single";
    if (RelationType.listValuedRelation(slotFields[1]))
      return "list";
    System.out.println("Invalid slot '" + slot + "'");
   ...
   */
  public static List<RelationType> relationsForNerType(String nerType) {
    nerType = nerType.toLowerCase();
    assert nerType.equals("per") || nerType.equals("org");

    List<RelationType> matching = new ArrayList<RelationType>();
    if (nerType.equals("per")) {
      for (RelationType relation : RelationType.PERRelations){ matching.add(relation); }
    } else if (nerType.equals("org")) {
      for (RelationType relation : RelationType.ORGRelations){ matching.add(relation); }
    }
    return matching;
  }
