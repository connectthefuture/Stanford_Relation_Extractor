...
//import edu.stanford.nlp.stats.Histogram;
...
public class TrainingStatistics implements Serializable {
  ...
  public static class EnsembleStatistics implements Serializable {
    ...
    public EnsembleStatistics(EnsembleStatistics toClone) {
      this.statisticsForClassifiers = new LinkedList<>(toClone.statisticsForClassifiers);
    }
    ...
    public SentenceStatistics mean() {
      ...
      Counter<String> avePredictions = new ClassicCounter<>(MapFactory.<String, MutableDouble>linkedHashMapFactory());
      // Sum
  ...
  private IterableIterator<Pair<CoreMap,Counter<String>>> iterableFromList(final KBPProcess process,
    ...
    // Create Iterator
    return new IterableIterator<>(new Iterator<Pair<CoreMap,Counter<String>>>() {
  ...
  private Counter<String> highKLFromMean() {
    // Get confidences
    Counter<String> kl = new ClassicCounter<>(MapFactory.<String, MutableDouble>linkedHashMapFactory());
    for( Map<SentenceKey, EnsembleStatistics> impl : this.impl ) {
  ...
  private Counter<String> lowAverageConfidence() {
    // Get confidences
    Counter<String> lowConfidence = new ClassicCounter<>(MapFactory.<String, MutableDouble>linkedHashMapFactory());
    for( Map<SentenceKey, EnsembleStatistics> impl : this.impl ) {
      for ( Map.Entry<SentenceKey, EnsembleStatistics> entry : impl.entrySet() ) {
  ...
  private Counter<String> uniformRandom() {
    Counter<String> uniformRandom = new ClassicCounter<>(MapFactory.<String, MutableDouble>linkedHashMapFactory());
    for( Map<SentenceKey, EnsembleStatistics> impl : this.impl ) {
      for ( Map.Entry<SentenceKey, EnsembleStatistics> entry : impl.entrySet() ) {
...
//    log(new Histogram(weightCounter, 50).toString());  // removed to make the release easier (Histogram isn't in CoreNLP)
    ...
    for (Pair<String, Double> elem : Counters.toSortedListWithCounts(weightCounter, (o1, o2) -> {
      ...
      if (elem.second != 0.0 || weightCounter.totalCount() == 0.0 || weightCounter.size() <= numSamples) {  // ignore 0 probability weights
        keys.add(elem.first);
    ...
    SAMPLE_ITER: for (int i = 1; i <= numSamples; ++i) {  // For each sample
      ...
      if (weights.size() == 0) { continue; } assert totalCount >= 0.0; assert weights.size() == keys.size(); double target = random.nextDouble() * totalCount;
