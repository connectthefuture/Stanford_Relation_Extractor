...
import edu.stanford.nlp.kbp.common.KBPAnnotations.SlotMentionsAnnotation;
...
public class RelationMentionAnnotator implements Annotator {
  ...
  public void annotate(Annotation annotation) {
    for (CoreMap sentence : annotation.get(SentencesAnnotation.class)) {
      ...
      sentence.set(RelationMentionsAnnotation.class, new ArrayList<>(relationMentions));
      if(annotateMode == KBPProcess.AnnotateMode.ALL_PAIRS) {
        sentence.set(AllRelationMentionsAnnotation.class, allRelationMentions);
      }
      sentence.set(SlotMentionsAnnotation.class, new ArrayList<>(slotMentions));  // should re-set slot mentions
    }
  }

  @Override
  public Set<Requirement> requirementsSatisfied() {
    return new HashSet<>(Arrays.asList(new Requirement[]{new Requirement("postir.relationmentions")}));
  }

  @Override
  public Set<Requirement> requires() {
    return new HashSet<>(Arrays.asList(new Requirement[]{new Requirement("postir.entitymentions"), new Requirement("postir.slotmentions")}));
  }
  ...
  private Triple<List<RelationMention>, List<RelationMention>, List<EntityMention>> getRelationAndNewSlotMentions(
    ...
    List<RelationMention> relations = new ArrayList<>();        // The relation mentions (for the pivot entity) we will return
    List<RelationMention> allRelations = new ArrayList<>();    // if annotateMode == ALL_PAIRS, this will hold all pairwise relations
    List<EntityMention> validSlotMentions = new ArrayList<>();  // The new set of valid slot mentions, in case we find something from knownSlots
    ...
    Set<KBPair> extractedPairs = new HashSet<>();  // If an entity or fill appears twice in a sentence, only extract one of the pairs
    List<CoreLabel> tokens = sentence.get(TokensAnnotation.class);
    // Tokens which are known entities
    boolean[] entityMask = new boolean[tokens.size()];
    List<Span> entitySpans = new ArrayList<>();
    ...
    Map<Span, Collection<Pair<Boolean, KBPSlotFill>>> directSlotMatches = new HashMap<>();
    List<Span> directSlotMatchesKeys = new ArrayList<>();
    for (KBPSlotFill slot : knownSlots) {
      if (Props.KBP_VERBOSE) { logger.debug("attempting to match slot " + slot.key.relationName + ":" + slot.key.slotValue); }
    ...
    Collections.sort(directSlotMatchesKeys, (o1, o2) -> o2.size() - o1.size());
    ...
    OUTER: for (Span slotSpan : directSlotMatchesKeys) {
      ...
      Set<String> relationLabels = new HashSet<>();
      ...
      assert Utils.getNERTag(slotValue).isDefined();
      ...
      for (final EntityMention entityMention : entityMentions) {
        ...
        // Create KBPair
        KBPair extractedPair = KBPNew.from(entity).slotValue(slotValue.getExtentString()).slotType(Utils.getNERTag(slotValue).orCrash()).KBPair();
        // Add relation
    ...
    List<EntityMention> unabsorbedCandidateSlotMentions = new ArrayList<>();
    ...
    for (final EntityMention slotValue : unabsorbedCandidateSlotMentions) {
      ...
      List<Pair<EntityMention, Boolean>> mentionsToTreatAsEntities = new ArrayList<>();
      for (EntityMention entityMention : entityMentions) {
      ...
      for (final Pair<EntityMention, Boolean> entityMentionAndIsEntity : mentionsToTreatAsEntities) {
        ...
        if (Props.PROCESS_RELATION_NORMALIZECOREFSLOT) {
          Counter<String> normSlotVotes = new ClassicCounter<>();  // try to find a better slot from coref
        ...
        if (entityMentionAndIsEntity.second) {
          ...
          Counter<String> antecedentVotes = new ClassicCounter<>();
          Counter<NERTag> nerVotes = new ClassicCounter<>();
          ...
          if (antecedentVotes.size() > 0 && nerVotes.size() > 0) {
            entity = KBPNew.entName(Counters.argmax(antecedentVotes)).entType(Counters.argmax(nerVotes)).KBPEntity();
          } else if (nerVotes.size() > 0) {
            entity = KBPNew.entName(entityMentionAndIsEntity.first.getExtentString()).entType(Counters.argmax(nerVotes)).KBPEntity();
          } else {
            warn(RED, "unknown NER type for entity: " + entityMentionAndIsEntity.first.getExtentString());
            entity = KBPNew.entName(entityMentionAndIsEntity.first.getExtentString()).entType(NERTag.PERSON).KBPEntity();
          }
        }
        // Create relation
        NormalizedRelationMention rm = new NormalizedRelationMention(normSlot, this.entity.equals(entity) ? this.entity : entity,
            "RM" + relationMentionCount.incrementAndGet(), sentence, ExtractionObject.getSpan(
  ...
  private static List<Pair<Span, Boolean>> matchSlotInSentence(KBPSlotFill slot, CoreMap sentence, boolean[] entityMask) {
    List<Pair<Span, Boolean>> matchingSpans = new ArrayList<>();
  ...
  public static List<String[]> alternateSlotValues(KBPSlotFill fill) {
    List<String[]> alternateSlotValues = new ArrayList<>();
    for( RelationType relation : fill.key.tryKbpRelation() ) {
      ...
      Collections.sort(alternateSlotValues, (o1, o2) -> {
  ...
  private Maybe<NERTag> voteOnSpanNEType(List<CoreLabel> tokens, Span span) {
    Counter<NERTag> votes = new ClassicCounter<>();
    for (int k : span) {
  ...
  private static final Set<String> PERSON_PREFIXES = new HashSet<>(Arrays.asList(new String[]{
      "mr", "mr.", "ms", "ms.", "mrs", "mrs.", "miss", "mister", "sir", "dr", "dr."
  }));
  private static final Set<String> PERSON_SUFFIXES = new HashSet<>(Arrays.asList(new String[]{
