...
public class GraphConsistencyPostProcessors {
  ...
  public static class EntityMergingPostProcessor extends GraphConsistencyPostProcessor {
    ...
    public EntityGraph postProcess(EntityGraph graph, GoldResponseSet checklist) {
      ...
      // (variables)
      Map<KBPEntity,Set<KBPEntity>> equivalenceClasses = new IdentityHashMap<>();
      List<KBPEntity> entities = new ArrayList<>(graph.getAllVertices());
      ...
      Counter<KBPEntity> priorities = new ClassicCounter<>();
      ...
      for (int first = 0; first < entities.size(); ++first) {
        ...
        for (int second = first + 1; second < entities.size(); ++second) {
          ...
          if (Props.ENTITYLINKING_LINKER.get().sameEntity(contexts.get(first), contexts.get(second))) {
    ...
    protected Map<KBPEntity, KBPRelationProvenance> getProvenanceMap(DirectedMultiGraph<KBPEntity, KBPSlotFill> graph, Set<KBPEntity> eqClass, KBPEntity representative) {
      Map<KBPEntity, KBPRelationProvenance> provenanceMap = new HashMap<>();
      for( KBPEntity entity : eqClass ) {
  ...
  public static class SymmetricFunctionRewritePostProcessor extends GraphConsistencyPostProcessor {
    ...
    public EntityGraph postProcess(EntityGraph graph, GoldResponseSet checklist) {
      ...
      for( Triple<KBPEntity,KBPEntity,List<KBPSlotFill>> edge : CollectionUtils.groupedEdges(graph) ) {
        ...
        List<KBPSlotFill> additionalSlots = new ArrayList<>();
        ...
        for( KBPSlotFill fill : edge.third ) {
          for( RelationType reln : fill.key.tryKbpRelation() ) {
            if( symmetricPairs.containsKey(reln) ) {
              ...
              if( !CollectionUtils.find( graph.getOutgoingEdges(tail), in -> in.key.tryKbpRelation().equalsOrElse( reln_, false )).isDefined() ) {
                // No? Let's add it
  ...
  public static class UnaryConsistencyPostProcessor extends GraphConsistencyPostProcessor {
    ...
    public EntityGraph postProcess(EntityGraph graph, final GoldResponseSet goldResponses) {
      return EntityGraph.transformEdges(graph, in -> {
  ...
  /**
   * <p>
   *   A post-processor implementing sentence level competition -- in the same vein
   *   as {@link edu.stanford.nlp.kbp.slotfilling.process.RelationFilter}.
   * </p>
   *
   * <p>
   *  In particular, the hypothesis is that for every sentence, for every slot value, for every
   *  relation holding with that slot value, there is only one entity in that relation.
   *  More precisely, if we have (e_1, r, v) and (e_2, r, v) predicted, we would like to claim that only one
   *  of the two should be accepted.
   * </p>
   *
   * TODO(gabor) what happens if we have multiple provenances for a slot fill? We can only chose one to add as provenance for the edge.
   */
  public static class SentenceCompetitionPostProcessor extends GraphConsistencyPostProcessor {
    /** The information to uniquely identify a sentence. I miss Scala's case classes so much... */
    private static class SentenceIdentifier {
      public final String indexName; public final String docid; public final int sentenceIndex;
      private SentenceIdentifier(String indexName, String docid, int sentenceIndex) { this.indexName = indexName; this.docid = docid; this.sentenceIndex = sentenceIndex; }
      @Override  // no one cares. It's the auto-generated equals().
      public boolean equals(Object o) { if (this == o) return true; if (!(o instanceof SentenceIdentifier)) return false; SentenceIdentifier that = (SentenceIdentifier) o; return sentenceIndex == that.sentenceIndex && docid.equals(that.docid) && indexName.equals(that.indexName); }
      @Override  // no one cares about this either. It's the auto-generated hashCode().
      public int hashCode() { int result = indexName.hashCode(); result = 31 * result + docid.hashCode(); result = 31 * result + sentenceIndex; return result; }
    }

    @Override
    public EntityGraph postProcess(EntityGraph graph, GoldResponseSet checklist) {
      Counter<Triple<SentenceIdentifier, String, String>> bestScores = new ClassicCounter<>();
      Iterator<KBPSlotFill> iter = graph.edgeIterator();
      while (iter.hasNext()) {
        KBPSlotFill edge = iter.next();
        if (edge.key.hasKBPRelation()) {  // only filter for KBP relations
          for (KBPRelationProvenance provenance : edge.provenance) {
            for (int sentenceIndex : provenance.sentenceIndex) {
              SentenceIdentifier sentence = new SentenceIdentifier(provenance.indexName, provenance.docId, sentenceIndex);
              Triple<SentenceIdentifier, String, String> candidate = Triple.makeTriple(sentence, edge.key.relationName, edge.key.slotValue);
              double score = edge.score.getOrElse(0.0) - (1e-3 * ((double) provenance.distanceBetweenEntities().getOrElse(5)));  // prefer closer entities
              if (bestScores.getCount(candidate) > score) {
                // case: another (relation, slotValue) pair already has a higher score in this sentence
                logger.log("filtered " + edge.key + " from sentence " + CoreMapUtils.sentenceToProvenanceString(provenance.containingSentenceLossy.orNull()));
                iter.remove();
                if (edge.key.entityId.isDefined()) { checklist.discardInconsistent(edge); }  // discard response if an official entity
              } else {
                // case: this is the highest scoring [or first] (relation, slotValue) pair in this sentence
                bestScores.setCount(candidate, score);
              }
            }
          }
        }
