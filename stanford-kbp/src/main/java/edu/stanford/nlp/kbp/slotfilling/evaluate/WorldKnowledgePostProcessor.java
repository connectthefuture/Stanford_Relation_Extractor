...
//  public final Gazetteer faustGazetteer;
  ...
  private static final Map<RelationType,RelationType> cityRewrite = new HashMap<>();
  private static final Map<RelationType,RelationType> regionRewrite = new HashMap<>();
  private static final Map<RelationType,RelationType> countryRewrite = new HashMap<>();
  static {
    { // City stuff
...
//    this.faustGazetteer = FaustGazetteer.getDefaultGazetteer();
    ...
    Interner<String> stringInterner = new Interner<>();
    String line;
    try {
      // Countries
      ...
      while ((line = cityReader.readLine()) != null) {
        ...
        if (regions == null) { regions = new HashSet<>(); city2region.put(city, regions); }
        if (region != null) {
          regions.add(region);
          Set<String> countries = region2country.get(region);
          if (countries == null) { countries = new HashSet<>(); region2country.put(region, countries); }
  ...
  public Maybe<String> countryForCity(String city) {
    return regionForCity(city).flatMap(in -> countryForRegion(in));
  }
   ...
   */
  public static synchronized WorldKnowledgePostProcessor singleton(File dir) {
