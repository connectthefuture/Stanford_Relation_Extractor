...
public class PerceptronExtractor extends RelationClassifier {
  ...
  final edu.stanford.nlp.kbp.slotfilling.classify.ModelType modelType;
  ...
  public TrainingStatistics train(KBPDataset<String, String> dataset) {
    log("Training the \"at least once\" model using "
    ...
    for(int t = 0; t < epochs; t ++){
      ...
      for(int i = 0; i < dataset.size(); i ++){
        ...
        if(modelType == edu.stanford.nlp.kbp.slotfilling.classify.ModelType.AT_LEAST_ONCE_INC) {
          trainJointlyOneGroupIncomplete(crtGroup, goldPos, goldNeg);
        } else if(modelType == edu.stanford.nlp.kbp.slotfilling.classify.ModelType.PERCEPTRON) {
      ...
      log("Epoch #" + t + " completed. Inspected " +
    ...
    log("Run model through " + iterations + " iterations.");
  ...
  private Counter<Integer> toProbabilities(Counter<Integer> scores) {
    ...
    for(Integer l: scores.keySet()) {
      ...
      double prob = softmax(score, allScores, gamma);
  ...
  private void computeTrainingPerformance(KBPDataset<String, String> dataset) {
    ...
    log("Overall score: P " + overallScore.first() +
  ...
  private Counter<String> classifyLocally(Collection<String> testDatum) {
    ...
    for(Integer z: allLabelScores.keySet()){
      ...
      result.setCount(l, softmax(allLabelScores.getCount(z), scores, gamma));
    }

    return result;
  }

  public static PerceptronExtractor load(String modelPath, Properties props) throws IOException, ClassNotFoundException {
    return RelationClassifier.load(modelPath, props, PerceptronExtractor.class);
  }
}
