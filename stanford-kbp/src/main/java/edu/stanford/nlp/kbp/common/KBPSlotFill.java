...
public class KBPSlotFill implements Serializable, Comparable<KBPSlotFill> {
  ...
  public KBPProtos.KBPSlotFill toProto() {
    KBPProtos.KBPSlotFill.Builder builder = KBPProtos.KBPSlotFill.newBuilder();
    builder.setKbEntry(key.toProto());
    for (KBPRelationProvenance provenance : this.provenance) { builder.setProvenance(provenance.toProto()); }
    for (double score : this.score) { builder.setScore(score); }
  ...
  public String toString() {
    if(score.isDefined())
      return key.entityName + ", " + key.relationName + "[" + score.getOrElse(0.0) + "] : " + key.slotValue;
    else
