...
public class FeatureFactory implements Serializable {
  ...
  private static final Redwood.RedwoodChannels logger = Redwood.channels("Featurizer");

  public static final String COREF_FEATURE = "is_coref";
  ...
  private static final List<String> dependencyFeatures = Collections.unmodifiableList(Arrays.asList(
      ...
      "verb_in_dependency_path","dependency_path","dependency_path_words","dependency_path_clusters","dependency_paths_to_verb",
  ...
  private static final Map<String, String> clusters = new HashMap<String, String>(){{
    try {
      BufferedReader reader = IOUtils.getBufferedReaderFromClasspathOrFileSystem(Props.PROCESS_WORDCLUSTERS_FILE.getPath());
      String line;
  ...
  public Datum<String,String> createDatum(RelationMention rel) {
    ...
    return new BasicDatum<>(features, labelString);
  }
  ...
  public boolean addFeatures(Collection<String> features, RelationMention rel, List<String> types) {
    Collection<String> rawFeatures = new ArrayList<>();
  ...
  private String postProcessFeature(String feat) {
    ...
    return whitespace.matcher(feat).replaceAll("_");
  }
  // END GRAFT
  ...
  private boolean addFeaturesRaw(Collection<String> features, RelationMention rel, List<String> types) {
    ...
    String arg0Type = findTrueNERTag(arg0.getType(), rel.getType());
      ...
      if (true) throw new IllegalStateException("If you got here from KBP, I've changed the way EntityMentionsAnnotation works. --Gabor");
  		String feature;
    ...
    // arg_clusters:  Clusters fore each token in the argument
    if (usingFeature(types, checklist, "arg_cluster")) {
      if (clusters.containsKey(word0)) { features.add("cluster_arg0: " + clusters.get(word0)); }
      if (clusters.containsKey(word1)) { features.add("cluster_arg1: " + clusters.get(word1)); }
      if (clusters.containsKey(word0) && clusters.containsKey(word1)) {
        features.add("cluster_args: " + clusters.get(word0) + "__" + clusters.get(word1));
      }
    }
    ...
    // adjacent_clusters: words (mapped to clusters) immediately to the left and right of the args
    if(usingFeature(types, checklist, "adjacent_clusters")){
      for(int i = 0; i < rel.getArgs().size(); i ++){
        Span s = ((EntityMention) rel.getArg(i)).getHead();
        if(s.start() > 0){
          String v = tokens.get(s.start() - 1).word();
          if (clusters.containsKey(v)) { features.add("leftarg" + i + "-" + clusters.get(v)); }
        }
        if(s.end() < tokens.size()){
          String v = tokens.get(s.end()).word();
          if (clusters.containsKey(v)) { features.add("rightarg" + i + "-" + clusters.get(v)); }
        }
      }
    }
      ...
      List<EntityMention> allEntities = new ArrayList<>();  // Added by Gabor, as KBPReader now only adds the entity to the EntityMentionAnnotation
      allEntities.addAll(rel.getSentence().get(MachineReadingAnnotations.EntityMentionsAnnotation.class));
      if (rel.getSentence().containsKey(KBPAnnotations.SlotMentionsAnnotation.class)) {
        allEntities.addAll(rel.getSentence().get(KBPAnnotations.SlotMentionsAnnotation.class));
      }
  		for (EntityMention arg : allEntities) {
    ...
    if (rel.getSentence().containsKey(KBPAnnotations.SlotMentionsAnnotation.class)) { // Added by Gabor, as EntityMentions is now only the entity
      for (EntityMention arg : rel.getSentence().get(KBPAnnotations.SlotMentionsAnnotation.class))
        typeCounts.incrementCount(arg.getType());
    }
    if (typeCounts.size() == 0) {
      throw new IllegalArgumentException("No entity mentions in: " + rel.getSentence());
    }
    ...
    // surface_path_clusters: concatenation of tokens between the args, mapped to word clusters
    ...
    StringBuilder sbClusters = new StringBuilder();
      ...
      if (clusters.containsKey(word)) { sbClusters.append(clusters.get(word)).append("_"); }
  	}
    ...
    if (usingFeature(types, checklist, "surface_path_clusters")) {
      features.add("surface_path_clusters: " + sbClusters);
    }
    ...
    // span_clusters_unigrams: span words, mapped to their word clusters
    if (usingFeature(types, checklist, "span_clusters_unigrams")) {
      for(int i = swStart; i < swEnd; i ++){
        String word = tokens.get(i).word();
        if (clusters.containsKey(word)) {
          features.add("span_cluster:" + clusters.get(word));
        }
      }
    }
    ...
    if (usingFeature(types, checklist, COREF_FEATURE)) {
      if (rel.getSentence().containsKey(KBPAnnotations.IsCoreferentAnnotation.class) &&
          rel.getSentence().get(KBPAnnotations.IsCoreferentAnnotation.class)) {
        features.add(COREF_FEATURE);
      }
    }
   ...
   * @param relType
   */
  private static String findTrueNERTag(String origEntType, String relType) {
    ...
    if(relType.startsWith("org:") && origEntType.contains("PERSON")) {
      String type = Utils.makeNERTag(NERTag.ORGANIZATION);
      logger.debug("TYPE CHANGE: from " + origEntType + " to " + type);
      return type;
    }

    else if(relType.startsWith("per:") && origEntType.contains("ORGANIZATION")) {
      String type = Utils.makeNERTag(NERTag.PERSON);
      logger.debug("TYPE CHANGE: from " + origEntType + " to " + type);
      return type;
    }
  ...
  protected void addDependencyPathFeatures(
    ...
    if(graph == null){
  	...
  	if (graph == null){
  	  logger.err("Cannot find dependency graph for dependencyType " + dependencyType + " in the sentence below.");
      logger.err("Current sentence from index " + rel.getSentence().get(SourceIndexAnnotation.class) + " is: " + CoreMapUtils.sentenceToString(rel.getSentence(), true, true, true, true, true, true, false));
  	  assert(false);
//  	} else {
//  	  logger.debug("Dependency graph: " + graph);
  	}
    ...
    LinkedList<IndexedWord> pathNodes = null;
    if (edgePath != null) {
      pathNodes = new LinkedList<>();
      pathNodes.add(node0);
      for (SemanticGraphEdge edge : edgePath) {
        if (pathNodes.getLast().equals(edge.getTarget())) {
          pathNodes.addLast(edge.getSource());
        } else if (pathNodes.getLast().equals(edge.getSource())) {
          pathNodes.addLast(edge.getTarget());
        } else {
          throw new IllegalStateException("Path is not contiguous?");
        }
      }
    }
    ...
    if (usingFeature(types, checklist, "dependency_path_lowlevel")) {
  	...
  	if (usingFeature(types, checklist, "different_sentences") && pathDescription.contains("fakeroot")) {
      features.add("different_sentences");
    }

    List<String> pathLemmas = new ArrayList<>();
    List<String> noArgPathLemmas = new ArrayList<>();
    List<String> noArgPathWords = new ArrayList<>();
    ...
    for(int i = arg1.getExtentTokenStart(); i < arg1.getExtentTokenEnd(); i ++) indecesToSkip.add(i + 1);
      ...
      pathLemmas.add(lemma);
  	  if(! indecesToSkip.contains(node.index())) {
  	    noArgPathLemmas.add(lemma);
        noArgPathWords.add(node.value());
      }
  	}
    ...
    // dependency_path_clusters: generates a feature for each word in the dependency path (mapped to word clusters)
    ...
    if (usingFeature(types, checklist, "dependency_path_clusters")) {
      for (String word : noArgPathWords) {
        if (clusters.containsKey(word)) { features.add("cluster_in_dependency_path:" + clusters.get(word)); }
      }
    }
