...
public class GoldResponseSet {
  ...
  public static class GoldResponse {
    ...
    public String toString() {
      Set<String> onlySlotValues = new HashSet<>();
  ...
  private Map<GoldResponse, GoldResponse> goldResponses = new HashMap<>();
  /** The set of guessed responses (for the checklist functionality */
  private Set<GuessResponse> guessedResponses = new HashSet<>();
  /** The set of discarded responses, that we've changed our mind on during consistency */
  private Set<Pair<GuessResponse,ErrorType>> discardedResponses = new HashSet<>();
  ...
  /**
   * Create a new gold response set, using the default test entities.
   * Obviously, this only works if the "test" set has already been evaluated and we know the responses.
   */
  ...
  /**
   * A copy constructor, used internally.
   */
  private GoldResponseSet(HashMap<GoldResponse, GoldResponse> goldResponses) {
    this.entities = Maybe.Nothing();
    this.goldResponses = goldResponses;
  }

  /**
   * Create a new gold response set, using only the entities provided.
   *
   * @param entities The entities to read in correct responses for.
   */
  ...
  private void readGoldResponses(Collection<KBPOfficialEntity> queries, ResponseProcessor processor) {
    if (!Props.TEST_RESPONSES.isDefined()) {
      log(YELLOW, "WARNING: No gold responses (ok if live evaluation)!");
      return;
    }
    startTrack("Getting gold responses");
    try {
      Map<String, KBPOfficialEntity> queryIdToEntity = new HashMap<>();
      for (KBPOfficialEntity entity : queries) {
        queryIdToEntity.put(entity.queryId.orCrash(), entity);
      }
      for (File queryFile : DataUtils.fetchFiles(Props.TEST_RESPONSES.orCrash().getPath(), "", false)) {
  ...
  /**
   * Return whether this KBTriple is known to be correct.
   * Note that this is a linear scan over the response file, and is therefore slow and should be used
   * sparingly.
   * @param triple The triple to check for inclusion.
   * @return True if the response file has marked this triple to be correct (for any provenance)
   */
  public boolean isTrue(KBTriple triple) {
    KBPEntity entity = triple.getEntity();
    for (GoldResponse gold : goldResponses.values()) {
      if (!gold.correctSlotValues.isEmpty() && gold.entity.equals(entity) && gold.relation == triple.tryKbpRelation().orNull()) {
        for (Pair<String, String> slotEntry : gold.correctSlotValues) {
          if (slotEntry.first.equals(triple.slotValue)) { return true; }
        }
      }
    }
    return false;
  ...
  /**
   * Return whether this KBTriple is known to be incorrect.
   * Note that this is a linear scan over the response file, and is therefore slow and should be used
   * sparingly.
   * @param triple The triple to check for inclusion.
   * @return True if the response file has marked this triple to be incorrect (for any provenance)
   */
  public boolean isFalse(KBTriple triple) {
    KBPEntity entity = triple.getEntity();
    for (GoldResponse gold : goldResponses.values()) {
      if (!gold.incorrectSlotValues.isEmpty() && gold.entity.equals(entity) && gold.relation == triple.tryKbpRelation().orNull()) {
        for (Pair<String, String> slotEntry : gold.incorrectSlotValues) {
          if (slotEntry.first.equals(triple.slotValue)) { return true; }
        }
      }
    }
  ...
  public Map<KBPSlotFill, Set<String>> correctProvenances() {
    if (correctProvenanceCached == null) {
      correctProvenanceCached = new HashMap<>();
  ...
  private boolean keyFileWritten = false;

  /**
   * Return a file containing the responses read from the given year, in the format of the 2010 evaluation
   * (thus, in a format that can be evaluated with {@link CustomSFScore}).
   * If the key file already exists, it does not recompute it.
   *
   * @return A file containing the key for the given year's gold responses, in 2010 format.
   */
  public synchronized File keyFile() {
    File keyFile = new File(Props.WORK_DIR.getPath() + File.separator + "key.tab");
    // Don't write it multiple times
    if (keyFile.exists()) {
      if (keyFileWritten) {
        return keyFile;
      } else {
        if (!keyFile.delete()) { warn("could not remove key file"); }
      }
    }
    try {
   ...
   * In particular, correct slots discarded with {@link GoldResponse#discardResponse(edu.stanford.nlp.kbp.common.KBPSlotFill, edu.stanford.nlp.kbp.slotfilling.evaluate.GoldResponseSet.ErrorType)}
   ...
   * This can be undone with {@link GoldResponse#undoDiscardResponse(edu.stanford.nlp.kbp.common.KBPSlotFill, edu.stanford.nlp.kbp.slotfilling.evaluate.GoldResponseSet.ErrorType)}.
   ...
   * from {@link GoldResponseSet#discardResponse(edu.stanford.nlp.kbp.common.KBPSlotFill, edu.stanford.nlp.kbp.slotfilling.evaluate.GoldResponseSet.ErrorType)}.
  ...
  /**
   * Alias for {@link edu.stanford.nlp.kbp.slotfilling.evaluate.GoldResponseSet#discardResponse(edu.stanford.nlp.kbp.common.KBPSlotFill, edu.stanford.nlp.kbp.slotfilling.evaluate.GoldResponseSet.ErrorType)}
  ...
  /**
   * Alias for {@link edu.stanford.nlp.kbp.slotfilling.evaluate.GoldResponseSet#undoDiscardResponse(edu.stanford.nlp.kbp.common.KBPSlotFill, edu.stanford.nlp.kbp.slotfilling.evaluate.GoldResponseSet.ErrorType)}
   * with a cause of {@link edu.stanford.nlp.kbp.slotfilling.evaluate.GoldResponseSet.ErrorType#NO_PROVENANCE}.
   */
   ...
   */
  public void discardInference(KBPSlotFill fill) {
    discardResponse(fill, ErrorType.INFERENCE);
  }
   ...
   * @param fill
   */
  public void undoDiscard(KBPSlotFill fill) {
    if (fill.key.tryKbpRelation().isDefined()) {
      GuessResponse guess = new GuessResponse(fill);
      List<Pair<GuessResponse,ErrorType>> undos = new ArrayList<>();
      for(Pair<GuessResponse,ErrorType> entry : discardedResponses) {
        if( entry.first.equals(guess) )
          undos.add(entry);
      }
      for(Pair<GuessResponse,ErrorType> entry : undos) {
        discardedResponses.remove(entry);
      }
    }
  }
  ...
  public PrettyLoggable loggableForEntity(final KBPOfficialEntity entity, final Maybe<KBPIR> irComponent) {
    return new PrettyLoggable(){
      ...
      public void prettyLog(Redwood.RedwoodChannels channels, String description) {
        ...
        Set<GoldResponse> missingGoldResponses = new HashSet<>();
        Set<GoldResponse> knownIncorrectResponses = new HashSet<>();
        for (GoldResponse gold : goldResponses.values()) {
        ...
        int totalGuessed = 0;
        List<Pair<GuessResponse,Maybe<Boolean>>> responses = new ArrayList<>();
        Map<ErrorType, Set<GuessResponse>> filteredButCorrect = new HashMap<>();
        Set<GuessResponse> correctlyDiscardedResponses = new HashSet<>();
        for (ErrorType type : ErrorType.values()) { filteredButCorrect.put(type, new HashSet<GuessResponse>()); }
        Map<GuessResponse, Integer> clusterIds = new IdentityHashMap<>();
        ...
        Collections.sort(responses, (o1, o2) -> o1.first.compareTo(o2.first));
  ...
  public void appendForEntity(KBPOfficialEntity entity, Maybe<KBPIR> irComponent) {
    String outputFileName = Props.WORK_DIR.getPath() + File.separator + "results.out";
    try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(outputFileName, true)))) {
      ...
      // Print
      out.append("Score For ").append(entity.name).append("{\n");

      // Responses
      out.append("Responses ").append("{\n");
      Collections.sort(responses, (o1, o2) -> o1.first.slotValue.compareTo(o2.first.slotValue));
      for (Pair<GuessResponse, Maybe<Boolean>> response : responses) {
        String sym = "✘ ";
        if (response.second.isDefined()) {
          if (response.second.get()) {
        ...
        out.append(sym).append(response.first.toString()).append("\n");
        for (KBPRelationProvenance provenance : response.first.provenance) {
          for (KBPIR ir : irComponent) {
            if (provenance.isOfficial()) { out.append("Provenance: ").append(provenance.gloss(ir)).append("\n"); }
          }
        }
      }
      ...
      // Filtered
      for (Map.Entry<ErrorType, Set<GuessResponse>> filteredEntries : filteredButCorrect.entrySet()) {
        out.append("Correct but " + filteredEntries.getKey().name().replaceAll("_", " ").toLowerCase()).append(" {\n");
        for (GuessResponse response : filteredEntries.getValue()) {
          // Print missed response
          out.append(response.toString()).append("\n");
          // Print provenance (if the error was incorrect provenance)
          if (filteredEntries.getKey() == ErrorType.WRONG_PROVENANCE && response.provenance.isDefined() && irComponent.isDefined() &&
                  response.provenance.get().isOfficial()) {
            out.append(response.provenance.get().gloss(irComponent.get())).append("\n");
          }
        }
        out.append("} << Correct but " + filteredEntries.getKey().name().replaceAll("_", " ").toLowerCase()).append("\n\n");
      }
      ...
      out.append("Missing {\n");
      for (GoldResponse gold : missingGoldResponses) {
        out.append(gold.toString()).append("\n");
      }
      out.append("} << Missing\n\n");

      // Correctly Filtered
      out.append("Correctly Filtered {\n");
      for (GuessResponse discarded : correctlyDiscardedResponses) {
        out.append(discarded.toString()).append("\n");
      }
      out.append("} << Correctly Filtered\n\n");
      ...
      double f1 = 2.0 * p * r / (p + r);
      double f1AnyDoc = 2.0 * pAnyDoc * rAnyDoc / (pAnyDoc + rAnyDoc);
      out.append("NORMAL: P " + df.format(p) + " R " + df.format(r) + " F1 " + df.format(f1)).append("\n");
      out.append("ANYDOC: P " + df.format(pAnyDoc) + " R " + df.format(rAnyDoc) + " F1 " + df.format(f1AnyDoc)).append("\n");
      out.append("} << Score For " + entity.name).append("\n");
    } catch (IOException e) {
    }
  }
